...
// CUDA kernel function using algorithm as in svenpeter_convolve_iir_nosimd.cxx
__global__ void convolve_row( float* input, float* output,
                              int M, int N,
                              int n_border,
                              float* d, float* causal, float* anticausal )
{
  ...
  if (dim < M){
    ...
    // convolve row exactly as in svenpeter_convolve_iir_nosimd.cxx
...
// Transfers data to GPU and back and executes convolution with CUDA kernel
